...
const App: React.FC = () => {
  ...
  const netIncome =
    totalIncome - totalExpenses - VATPayable + totalVATDeductible;
  ...
  const handleEditIncome = (updatedIncome: Income) => {
    setIncome(
      income.map((inc) => (inc.id === updatedIncome.id ? updatedIncome : inc))
    );
    setEditingIncome(null);
  };
            ...
            <div className="p-6">
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    {["Date", "Source", "Amount (€)", "Actions"].map(
                      (header) => (
                        <th
                          key={header}
          ...
          >
            <div className="p-6">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {expenses.map((exp) => {
                    const { vatAmount, netPrice } = vatCalc(
                      exp.amount,
                      exp.VATRate
                    );
                    return (
